...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    type QualityKey = keyof typeof qualitySettings;
    let effectiveQuality: QualityKey = quality as QualityKey;
    ...
    const fragmentShader = `
      ...
      varying vec2 vUv;const float STEP_MULT=${settings.stepMultiplier.toFixed(1)};
      ...
      vec3 ro=vec3(0.0,0.0,-10.0);vec3 rd=normalize(vec3(uv,1.0));float rotC=uRotCos;float rotS=uRotSin;
      if(uInteractive&&(uMouse.x!=0.0||uMouse.y!=0.0)){float a=uMouse.x*6.283185;rotC=cos(a);rotS=sin(a);}
      vec3 col=vec3(0.0);float t=0.1;
      for(int i=0;i<MAX_ITER;i++){vec3 p=ro+rd*t;p.xz=vec2(rotC*p.x-rotS*p.z,rotS*p.x+rotC*p.z);
      vec3 q=p;q.y=p.y*uPillarHeight+uTime;float freq=1.0;float amp=1.0;
      for(int j=0;j<WAVE_ITER;j++){q.xz=vec2(uWaveCos*q.x-uWaveSin*q.z,uWaveSin*q.x+uWaveCos*q.z);
      q+=cos(q.zxy*freq-uTime*float(j)*2.0)*amp;freq*=2.0;amp*=0.5;}
      float d=length(cos(q.xz))-0.2;float bound=length(p.xz)-uPillarWidth;float k=4.0;
      float h=max(k-abs(d-bound),0.0);d=max(d,bound)+h*h*0.0625/k;d=abs(d)*0.15+0.01;
      float grad=clamp((15.0-p.y)/30.0,0.0,1.0);col+=mix(uBottomColor,uTopColor,grad)/d;t+=d*STEP_MULT;if(t>50.0)break;}
  ...
  if (!webGLSupported)
    return (
      <div
        ...
        style={{ mixBlendMode: mixBlendMode as CSSProperties["mixBlendMode"] }}
      >
        WebGL not supported
      </div>
    );
